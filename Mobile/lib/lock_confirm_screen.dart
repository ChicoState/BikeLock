...
class lock_confirm_screen_state extends State<lock_confirm_screen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.lock.lockName), centerTitle: true),
      body: Center(
        ...
        SizedBox(height: 50),
        Text(
          widget.lock.lockDescription,
          style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),
        ),
        SizedBox(height: 50),
        Text(
          widget.lock.belongsToRack,
          style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),
        ),
        SizedBox(height: 50),
        Text(
  ...
  lock_and_unlock(Lock lock) {
    ...
    developer.log("This lock state: " + widget.lock.lockName);
